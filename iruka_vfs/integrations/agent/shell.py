...
from iruka_vfs.integrations.agent.guidance import (
    GUIDANCE_STATE_KEY,
    assemble_shell_guidance_payload,
    build_task_guidance,
)
from iruka_vfs.integrations.agent.path_guidance import build_workspace_path_guidance
...
def run_virtual_bash(
    ...
    *,
    ...
    try:
        ...
        task_guidance = transaction["task_guidance"]
        workspace_outline = service.render_virtual_tree(db, workspace.id, max_depth=3)
        path_guidance = build_workspace_path_guidance(
            service,
            db,
            workspace,
            workspace_outline=workspace_outline,
        )
        shell_guidance = assemble_shell_guidance_payload(
        ...
        result_artifacts = shell_guidance.artifacts
    ...
    return {
        ...
        "cwd": cwd_path,
        **shell_guidance.response_fields,
    }
...
def _execute_virtual_bash_transaction(service, db: Session, workspace: Any, tenant_key: str, scope_key: str | None, raw_cmd: str):
    ...
    def execute(mirror, _workspace_ref):
        ...
        previous_guidance_state = dict((mirror.workspace_metadata or {}).get(GUIDANCE_STATE_KEY) or {})
        task_guidance = build_task_guidance(previous_guidance_state, dict(result.artifacts or {}))
        if task_guidance.state != previous_guidance_state:
            metadata = dict(mirror.workspace_metadata or {})
            metadata[GUIDANCE_STATE_KEY] = task_guidance.state
            mirror.workspace_metadata = metadata
            mirror.dirty_workspace_metadata = True
            mirror.revision += 1
